...
if (typeof(EventTypes) == "undefined") {//devuelve una cadena que indica el tipo del operando sin evaluarlo
  EventTypes = {};
}
...
// le asigna a cada evento del navegador un valor
function CasperRenderer(document) { // funcion que define el formato del documento, calculo que el script
...
CasperRenderer.prototype.text = function(txt) {// transforma un texto en un document.writeln
  // todo: long lines
  this.document.writeln(txt);
}

CasperRenderer.prototype.stmt = function(text, indent) { // identa el texto que tranformaste
...
CasperRenderer.prototype.pyrepr = function(text, escape) {//a mi entender controla si ocurre un escape dentro de los eventos que son gravados
...
CasperRenderer.prototype.space = function() { // añade espacios en blanco 
  this.document.write("\n");
}
...
CasperRenderer.prototype.render = function(with_xy) {
  ...
  this.document.open();
  /*this.document.writeln('<button id="casperbox-button">Descargar txt</button>');*/
  this.document.write("<" + "pre" + ">");
  this.writeHeader();
  var last_down = null;
  var forget_click = false;
//realiza la grabacion del script
  ...
  this.writeFooter();
  console.log(this.document.getElementsByTagName("pre")[0].innerHTML)
  this.document.close();
  descargarArchivo(generarTexto(this.document.getElementsByTagName("pre")[0].innerHTML),"Archivo.js");
} 

CasperRenderer.prototype.writeHeader = function() {
  var date = new Date();
  this.text("/*==============================================================================*/", 0);
  this.text("/* Grabador Tremo " + date + " */", 0);
  this.text("/*==============================================================================*/", 0);
  this.space();
...
CasperRenderer.prototype.startUrl = function(item) {
  ...
  this.stmt("}", 1);
  this.stmt("});", 0);
  this.stmt("casper.test.begin('Tremo test', function(test) {", 0);
...
var dt = new CasperRenderer(document);//devuelve el script de coordenadas
